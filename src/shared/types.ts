...
export interface SaleWithDetails extends Sale {
  ...
  items: (SaleItem & { product_name: string; color_name: string; color_hex: string; package_name?: string })[];
}
...
export interface Payment {
  ...
  note: string | null;
  sale_id?: number | null;
  is_initial?: boolean | number; // boolean from API, but sqlite returns number 0/1
  customer_name?: string;
  created_at: string;
  updated_at: string;
}
...
export interface OrderWithItems extends Order {
  ...
  items: (OrderItem & { product_name: string; color_name: string; color_hex: string })[];
}
